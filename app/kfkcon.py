...
th_queue_get = None
topic_send = 'tp.test.common'
topic_get = 'tp.test.common.response'
th_response = None
th_lock = Lock()

class KfkCon(Namespace):
    def doResponse(self):
        global th_queue_get
        ...
        while True:
            if not th_queue_get.empty():
                ...
                print('主题：{} 获取一条回复：{}'.format(topic_get, value))
    ...
    def on_connect(self):
        print('有个客户端{}链接到KafkaConnection'.format(request.sid))
        global th_queue_get
    ...
    def on_disconnect(self):
        print('有个客户端{}断开链接'.format(request.sid))

    def on_order(self, result):

        print('收到一条命令：{}'.format(result))
